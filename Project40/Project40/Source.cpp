...
void roundRobinScheduler(ProcessTable* processTable, int quantum)
{
	...
	while (true)
	{
		...
		for (int i = 0; i < processTable->numProcesses; i++)
		{
			if (processTable->process[i]->finished)
			{
				continue;
			}
			isDone = false;
			if (processTable->process[i]->arrivalTime > currentTime)
			{
				continue;
			}
			if (processTable->process[i]->totalTime + quantum >= processTable->process[i]->serviceTime)
			{
				int timeDiff = 0;
				timeDiff = processTable->process[i]->serviceTime - processTable->process[i]->totalTime;
				currentTime += timeDiff;
				processTable->process[i]->totalTime = processTable->process[i]->serviceTime;
				processTable->process[i]->finishTime = currentTime;
				processTable->process[i]->finished = true;
				for (int j = 0; j < timeDiff; j++)
				{
					cout << processTable->process[i]->processName << " ";
				}
			}
			else
			{
				currentTime += quantum;
				processTable->process[i]->totalTime += quantum;
	...
	cout << endl << "Name\tFnsh\tT_r\tT_r/T_S" << endl;
	cout << "-----------------------------------------" << endl;
	for (int i = 0; i < processTable->numProcesses; i++)
	{
		cout << processTable->process[i]->processName << "\t" << processTable->process[i]->finishTime << "\t" << processTable->process[i]->finishTime - processTable->process[i]->arrivalTime << "\t" << (double)(processTable->process[i]->finishTime - processTable->process[i]->arrivalTime) / (double) processTable->process[i] ->serviceTime << endl;
	}
...
void shortestRemainingTime(ProcessTable* processTable)
{
	...
	bool isDone = false;
	for (int a = 0; a < processTable->numProcesses; a++)
	{
		minRemainingIndex = 0;
		...
		for (int i = 0; i < processTable->numProcesses; i++)
		{
			if (processTable->process[i]->finished)
				continue;
			if (processTable->process[i]->arrivalTime<=currentTime && (processTable->process[i]->serviceTime - processTable->process[i]->totalTime) < minRemainingTime)
			{
				minRemainingTime = processTable->process[i]->serviceTime - processTable->process[i]->totalTime;
				minRemainingIndex = i;
			}

		}
		currentTime += minRemainingTime;
		processTable->process[minRemainingIndex]->totalTime = processTable->process[minRemainingIndex]->serviceTime;
		processTable->process[minRemainingIndex]->finishTime = currentTime;
		processTable->process[minRemainingIndex]->finished = true;
		for (int j = 0; j < minRemainingTime; j++)
		{
			cout << processTable->process[minRemainingIndex]->processName << " ";
		}

	}
	cout << endl;
	cout << "Name\tFnsh\tT_r\tT_r/T_S" << endl;
	cout << "-----------------------------------------" << endl;
	for (int i = 0; i < processTable->numProcesses; i++)
	{
		cout << processTable->process[i]->processName << "\t" << processTable->process[i]->finishTime << "\t" << processTable->process[i]->finishTime - processTable->process[i]->arrivalTime << "\t" << (double)(processTable->process[i]->finishTime - processTable->process[i]->arrivalTime) /(double) processTable->process[i]->serviceTime << endl;
