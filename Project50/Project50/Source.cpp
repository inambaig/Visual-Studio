...
void *mymalloc(size_t requested)
{
	...
	case First:
		return NULL;
	case Best:
		head->next = malloc(sizeof(struct memoryList));
		head->next->last = head;
		head->next->next = NULL;
		head->next->alloc = '1';
		head->next->size = requested + sizeof(struct memoryList);
		next = head->next;
		head->next->ptr = memadr + sizeof(struct memoryList);
		return head->next;
	case Worst:
		head->next = malloc(sizeof(struct memoryList));
		head->next->last = NULL;
